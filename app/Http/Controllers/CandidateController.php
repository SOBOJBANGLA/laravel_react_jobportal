...
public function getCandidateDetail() { 
    $userId = Auth::id(); 
    $candidateDetail = CandidateDetails::where('user_id', $userId)->first();
     if (!$candidateDetail) { return response()->json(['message' => 'Candidate details not found'], 404); 
    } 
    $candidateDetail->image = url($candidateDetail->image); 
    return response()->json($candidateDetail);
    }
